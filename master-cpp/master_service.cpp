...
static std::vector<std::string> kDataNodes = {
    "localhost:50052", "localhost:50053", "localhost:50054"
};
...
public:
    ...
    if (kDataNodes.empty())
        return Status(StatusCode::FAILED_PRECONDITION, "Sin DataNodes registrados");
    ...
    for (int64_t i=0;i<blocks;++i){
        BlockAssignment a;
        a.set_block_id(req->filename()+"#blk"+std::to_string(i));
        // elige primario al azar (ejemplo simple)
        std::string primary=kDataNodes[rng()%kDataNodes.size()];
        a.set_primary_dn(primary);
        // replicas
        int rep=std::max(1,req->replication());
        for(const auto& dn:kDataNodes){
            if(dn!=primary && a.replica_dns_size()<rep-1) *a.add_replica_dns()=dn;
        }
        asgs.push_back(a);
        *resp->add_assignments()=a;
    }
    ms_->SavePutPlan(req->filename(), asgs);
    return Status::OK;
}
